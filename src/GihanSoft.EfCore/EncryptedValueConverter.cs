...
namespace GihanSoft.EfCore {
    public class ValueToStringConverter : ValueConverter<IConvertible, string> {
        ...
        public ValueToStringConverter (Type type) : base (value => TtoStr (value), value => StrToT (value, type)) {
            if (!IsTypeValid (type))
    ...
    public class StringToEncryptedConverter : ValueConverter<string, string> {
        public StringToEncryptedConverter (ICrypto cryptographer, bool haveSalt) : base (v => v.Decode ().Encrypt (cryptographer, haveSalt).EncodeBase64 (),
